...
if (isset($_POST['update_contact'])) {
    $newContact = mysqli_real_escape_string($con, trim($_POST['new_contact']));
    $doctorId = $_SESSION['id'];

    $update = mysqli_query($con, "UPDATE doctors SET contactno = '$newContact' WHERE id = '$doctorId'");

    if ($update) {
        $_SESSION['msg'] = "✅ Номер телефона обновлён!";
        header("Location: ".$_SERVER['PHP_SELF']);
        exit();
    } else {
        echo "<div class='alert alert-danger'>Ошибка обновления: " . mysqli_error($con) . "</div>";
    }
}
...
<body>
    ...
    <div class="form-group">
        ...
        <input type="text" name="new_contact" class="form-control" value="<?php echo htmlspecialchars($doctorData['contactno']); ?>" required>
    </div>
    <button type="submit" name="update_contact" class="btn btn-primary">Сохранить</button>
</form>
