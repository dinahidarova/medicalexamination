...
$id = intval($_GET['id']); // Получаем ID специализации

if(isset($_POST['submit'])) {
    $docspecialization = $_POST['doctorspecilization'];
    // Исправляем имя таблицы (должно совпадать с вашей БД)
    $sql = mysqli_query($con, "UPDATE doctorspecilization SET doctorSpecialization='$docspecialization' WHERE doctorSpecializationId='$id'");
    
    if($sql) {
        $_SESSION['msg'] = "Специализация врача успешно обновлена!";
    } else {
        $_SESSION['msg'] = "Ошибка обновления: " . mysqli_error($con);
    }
} 
?>
<!DOCTYPE html>
<html lang="en">
	<head>
		<title>Админ | Редактирование специализации врача</title>
